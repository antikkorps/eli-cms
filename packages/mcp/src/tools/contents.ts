...
export function registerContentTools(server: McpServer, api: ApiClient) {
  server.tool(
    ...
    {
      contentTypeId: z.string().uuid().optional().describe('Filter by content type ID'),
      status: z
      ...
      sortBy: z.enum(['createdAt', 'updatedAt', 'status', 'slug', 'relevance']).optional().describe('Sort field'),
  ...
  server.tool(
    ...
    'Create a new content entry. The data object must match the field definitions of the content type. Always get_content_type first to know the expected fields.',
    {
      ...
      data: z
        .record(z.unknown())
        .describe('Content data — keys must match the field names defined in the content type'),
      publishedAt: z.string().optional().describe('ISO datetime for scheduled publishing'),
    },
    async (params) => {
      try {
